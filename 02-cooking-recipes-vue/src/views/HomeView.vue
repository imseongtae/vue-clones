...
const newRecipeToDefault = () => {
  newRecipe.value.slug = '';
  newRecipe.value.title = '';
  newRecipe.value.description = '';
  newRecipe.value.ingredients = [''];
  newRecipe.value.method = [''];
  newRecipe.value.ingredientsRows = 1;
  newRecipe.value.methodRows = 1;
};
...
const removeMethod = (idx: number) => {
  newRecipe.value.method = newRecipe.value.method.filter(
    (_, id) => id !== idx - 1,
  );
  newRecipe.value.methodRows--;
};
...
const addNewRecipe = () => {
  ...
  recipesStore.addRecipe({ ...newRecipe.value });
  closeModal();
};

/**
 * @refactoring Pinia action을 사용하도록 변경
 * @param e Event
 */
const deleteRecipe = (e: Event) => {
  ...
  if (selectedRecipeSlug) {
    recipesStore.removeRecipe(selectedRecipeSlug);
  }
};
</script>
...
<template>
  <div class="home">
    <h1>My Recipes</h1>
    <button type="button" @click="openModal">Add new recipe</button>
    <div class="recipes">
      <div
        class="card"
      ...
      <div class="popup-content">
        ...
        <form @submit.prevent="addNewRecipe">
          ...
          <div class="group">
            ...
            <div
              class="ingredient"
              v-for="i in newRecipe.ingredientsRows"
              :key="i"
            >
